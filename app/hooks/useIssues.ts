...
const useIssues = (settings: UserSettings) => {
  ...
  const fetchIssues = useCallback(async (showNotifications = false, forceRefresh = false) => {
    ...
    try {
      ...
      for (const repo of settings.repositories) {
        ...
        try {
          // 레포지토리의 이슈 가져오기
          const repoIssues = await getIssues(repo, settings.customLabels, settings.hideClosedIssues);
